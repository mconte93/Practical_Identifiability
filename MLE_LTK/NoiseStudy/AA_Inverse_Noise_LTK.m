%Code to run the inverse problem on the LTK model with insilico data and
%insilico VIF with different levels of noise to obtain the optimal 
% %estimation of the model parameters

clear all
close all

ktr_0=.05;
ve_0=.01;
lambda_0=.005;
vp_0=.01;

%Optimization of the parameter values with PS algorithm
x0=[ktr_0 ve_0 vp_0 lambda_0];
fun=@fun_LTK_PS_AA;
lb = [0 1.e-3 1.e-3 0];
ub = [1 1 1 1];

options=optimoptions('particleswarm','SwarmSize',200,'HybridFcn',@fmincon,'MaxIterations',500,'ObjectiveLimit',1e-16,'InitialSwarmMatrix',x0,'PlotFcn',@pswplotbestf);
nvar=4;
[x,fval,exitflag,output] = particleswarm(fun,nvar,lb,ub,options);
formatstring = 'particleswarm reached the value %f using %d function evaluations.\n';
fprintf(formatstring,fval,output.funccount)

%Saving option
%save('AA_Inverse_Persistent_15Noise.mat','x')

function [MLE] = fun_LTK_PS_AA(x)

%Function to evaluate the MLE function

   %Load the data 
   load('AA_LTK_Persistent_15Noise.mat');
   ydata=C_for; 
   xdata=t_for;
   
   %Model parameter
   ktr=x(1);
   ve=x(2);
   vp=x(3);
   lambda=x(4);
   
   par_temp=[ktr,ve,vp,lambda];

   %Initial conditions
   Ce_0=0;
   Cl_0=0;  
   Init=[Ce_0;Cl_0];
   
   %Forward LTK model
   [t,y] = ode45(@(t,y) LTK_ODE(t,y,par_temp,VIF,xdata),xdata,Init,[]);
   C_approx=y(:,1)+y(:,2)+vp*VIF';
   
   %MLE evaluation
   MLE=0;
   for i=1:size(C_approx,1)
       res=ydata(i)-C_approx(i);
       if ~isnan(res)
          MLE=MLE+res.^2;
       end
   end

    
end