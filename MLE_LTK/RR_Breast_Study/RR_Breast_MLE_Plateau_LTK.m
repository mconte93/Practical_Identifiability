...
%plateau curve in the RR-Breast case
...
load('RR_Breast_Inverse_Plateau.mat')
ktr_opt=x(1);
ve_opt=x(2);
vp_opt=x(3);
leakage_opt=x(4); 
...
leakage_vec=linspace(leakage_opt/2,leakage_opt*2,10);
...
%Saving options
%save('MLE_ktr_Plateau','MLE','R2','ktr_vec','MLE_opt','ktr_opt','R2_opt','xsave')
...
%Saving options
%save('MLE_ve_Plateau','MLE','R2','ve_vec','MLE_opt','ve_opt','R2_opt','xsave')
% 
...
%Saving options
%save('MLE_vp_Plateau2.mat','MLE','R2','vp_vec','MLE_opt','vp_opt','R2_opt','xsave')
...
options=optimoptions('particleswarm','SwarmSize',200,'HybridFcn',@fmincon,'MaxIterations',500,'ObjectiveLimit',1e-16,'InitialSwarmMatrix',x0,'PlotFcn',@pswplotbestf);
fun=@fun_LTK_PS_RR_Breast_leakagePl;
nvar=3;
[x,fval,exitflag,output]  = particleswarm(fun,nvar,lb,ub,options);
 
%Forward LTK model
par_vec=[x(1),x(2),x(3),leakage_vec(i)];
[t,y] = ode45(@(t,y) LTK_ODEReal(t,y,par_vec,VIF,xdata),xdata,Init,[]);
C=y(:,1)+y(:,2)+x(2)*VIF;
...
%Saving options
%save('MLE_leakage_Plateau.mat','SSE','R2','leakage_vec','SSE_opt','leakage_opt','R2_opt','xsave')